...
class EmailSettingController extends Controller
{
    ...
    public function index()
    {
        ...
        $stats = [
            ...
            'active' => $settings->where('is_active', true)->count(),
            'inactive' => $settings->where('is_active', false)->count(),
            'tested' => $settings->filter(fn ($s) => ! empty($s->test_results))->count(),
        ];

        return view('admin.settings.email.index', compact('settings', 'activeSettings', 'providers', 'stats'));
    }
