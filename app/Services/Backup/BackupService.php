...
class BackupService
{
    ...
    public function createBackup(array $options): Backup
    {
        ...
        if (!$backup) {
            $backupType = $options['backup_type'] ?? 'full';
            ...
            $backup = Backup::create([
                ...
                'status' => 'pending',
                'retention_days' => $options['retention_days'] ?? config('backup.default_retention_days', 30),
                'created_by' => $options['created_by'] ?? auth()->id(),
                'schedule_id' => $options['schedule_id'] ?? null,
            ]);
        } elseif (! empty($options['scope'])) {
            $backup->update([
                'scope' => $this->scopeResolver->normalize($options['scope'], $backup->backup_type),
            ]);
        }
        ...
        try {
            ...
            $scope = $this->scopeResolver->normalize(
                $backup->scope ?? $options['scope'] ?? null,
                $backup->backup_type
            );
            $options['scope'] = $scope;
    ...
    {
        $scope = $options['scope'] ?? $this->scopeResolver->normalize($backup->scope, 'full');
        ...
        if ($this->scopeResolver->shouldBackupDatabase($scope)) {
        ...
        if (! empty($this->scopeResolver->resolveFilePaths($scope))) {
            $this->log($backup, 'info', 'بدء نسخ الملفات');
            $filesPath = $this->createFilesBackup($backup, $options);
            if (is_dir($filesPath)) {
                $this->copyDirectory($filesPath, $backupDir . '/files');
            }
        }

        if (! empty($this->scopeResolver->resolveConfigFiles($scope))) {
            $this->log($backup, 'info', 'بدء نسخ الإعدادات');
            $configPath = $this->createConfigBackup($backup, $options);
            if (is_dir($configPath)) {
                $this->copyDirectory($configPath, $backupDir . '/config');
            }
        }
    ...
    public function createDatabaseBackup(Backup $backup, array $options): string
    {
        ...
        $backupDir = config('backup.temp_path', storage_path('app/backups'));
        ...
        if ($driver === 'sqlite') {
            return $this->createSqliteDatabaseBackup($backup, $connection, $path);
        }

        if ($driver !== 'mysql') {
            throw new \Exception("نسخ قاعدة البيانات غير مدعوم لسائق: {$driver}. استخدم MySQL أو SQLite.");
        }

        if (config('backup.database.prefer_mysqldump', true)) {
            $dumped = $this->tryMysqlDump($connection, $path);
            if ($dumped) {
                $this->log($backup, 'info', 'تم نسخ قاعدة البيانات عبر mysqldump');
        ...
        try {
            ...
            $chunkSize = (int) config('backup.database.chunk_size', 100);
            ...
            foreach ($tables as $table) {
                ...
                if ($rows->count() > 0) {
                    ...
                    foreach ($rows as $row) {
                        ...
                        $valArray = array_map(function ($val) use ($db) {
                            if ($val === null) {
                                return 'NULL';
                            }
    ...
    {
        $scope = $options['scope'] ?? $this->scopeResolver->normalize($backup->scope, $backup->backup_type);
        $paths = $this->scopeResolver->resolveFilePaths($scope);
        ...
        foreach ($paths as $index => $sourcePath) {
            $target = $backupDir . '/path_' . $index;
            $this->copyDirectory($sourcePath, $target);
        }

        $this->log($backup, 'info', 'تم نسخ الملفات بنجاح (' . count($paths) . ' مسار)');
    ...
    {
        $scope = $options['scope'] ?? $this->scopeResolver->normalize($backup->scope, $backup->backup_type);
        $configFiles = $this->scopeResolver->resolveConfigFiles($scope);
        ...
        foreach ($configFiles as $file) {
            ...
            if (file_exists($sourcePath)) {
                $destPath = $backupDir . '/' . str_replace(['/', '\\'], '_', $file);
    ...
    public function restoreBackup(Backup $backup, array $options = []): bool
    {
        ...
        try {
            ...
            $restoreMode = $options['restore_mode'] ?? $backup->backup_type;

            $this->log($backup, 'info', 'بدء استعادة (وضع: ' . $restoreMode . ')...');

            match ($restoreMode) {
                'database' => $this->restoreDatabase($this->findSqlFile($extractedDirectory) ?: $extractedPath),
                'files' => $this->restoreFiles($extractedDirectory),
                'config' => $this->restoreConfig($extractedDirectory),
                'full' => $this->restoreFull($extractedDirectory),
                default => throw new \Exception('نوع الاستعادة غير معروف'),
            };

            $this->log($backup, 'info', 'تمت الاستعادة بنجاح');
    ...
    public function cleanupExpiredBackups(): int
    {
        ...
        foreach (Backup::expired()->get() as $backup) {
            try {
        ...
        if (config('backup.enforce_max_backups_per_storage', true)) {
            $count += $this->cleanupExcessBackupsByStorageLimit();
        }

        return $count;
    }

    protected function cleanupExcessBackupsByStorageLimit(): int
    {
        $deleted = 0;

        $configs = \App\Models\AppStorageConfig::where('is_active', true)
            ->whereNotNull('max_backups')
            ->where('max_backups', '>', 0)
            ->get();

        foreach ($configs as $config) {
            $excess = Backup::query()
                ->where('storage_config_id', $config->id)
                ->where('status', 'completed')
                ->orderByDesc('created_at')
                ->skip($config->max_backups)
                ->take(500)
                ->get();
            ...
            foreach ($excess as $backup) {
                try {
                    ...
                    Log::error('Error deleting excess backup: ' . $e->getMessage(), [
                        'backup_id' => $backup->id,
                    ]);
                }
            }
    ...
    protected function tryMysqlDump(array $connection, string $path): bool
    {
        $mysqldump = strtoupper(substr(PHP_OS, 0, 3)) === 'WIN' ? 'mysqldump.exe' : 'mysqldump';
    ...
    protected function createSqliteDatabaseBackup(Backup $backup, array $connection, string $path): string
    {
        $database = $connection['database'] ?? database_path('database.sqlite');

        if (! file_exists($database)) {
            throw new \Exception('ملف SQLite غير موجود: ' . $database);
        }

        if (! copy($database, $path)) {
            throw new \Exception('فشل نسخ ملف SQLite');
        }

        $this->log($backup, 'info', 'تم نسخ قاعدة بيانات SQLite');
