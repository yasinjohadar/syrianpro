...
class StorageHelperService
{
    ...
    {
        try {
            $directory = trim(str_replace('\\', '/', $path), '/');
            $category = $fileType ?? 'image';

            $result = match ($category) {
                'video' => MediaStorageService::uploadVideo($file, $directory),
                'document' => MediaStorageService::uploadDocument($file, $directory),
                default => MediaStorageService::uploadImage($file, $directory),
            };
            ...
            Log::error('StorageHelperService: MediaStorageService upload failed', [
                'disk' => $disk,
                'path' => $path,
                'result' => $result,
            ]);
