...
@php $authTab = 'login'; @endphp

@section('title', 'تسجيل الدخول — تك سوريا')
@section('auth-heading', 'مرحباً بعودتك')
@section('auth-subheading', 'سجّل دخولك للوصول لملفك ولوحة التحكم')
...
@section('content')
    ...
    <div class="auth-role-hint">
        <span class="auth-role-hint-label">تسجّل كـ:</span>
        <div class="auth-role-hint-chips">
            <span class="auth-role-chip auth-role-chip--talent">👤 تقني</span>
            <span class="auth-role-chip auth-role-chip--company">🏢 شركة</span>
        </div>
        <span class="auth-role-hint-note">نفس صفحة الدخول — يُوجّهك النظام حسب نوع حسابك</span>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="auth-form" novalidate>
        ...
        <button type="submit" class="btn btn-primary btn-full btn-lg auth-submit-btn">تسجيل الدخول</button>

        @if (Route::has('register'))
            <div class="auth-switch">
                ليس لديك حساب؟ <a href="{{ route('register') }}">إنشاء حساب مجاناً</a>
            </div>
        @endif
    </form>
