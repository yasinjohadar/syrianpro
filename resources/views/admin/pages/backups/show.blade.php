...
<div class="main-content app-content">
    <div class="container-fluid">
        ...
        @include('admin.partials.ui.page-header', [
            ...
            'title' => 'تفاصيل النسخة',
            'subtitle' => \App\Models\Backup::BACKUP_TYPES[$backup->backup_type] ?? $backup->backup_type,
            ...
            <div class="col-lg-8">
                <div class="card custom-card form-card mb-4">
                    <div class="card-header">
                        <h6 class="mb-0 fw-semibold fs-15"><i class="ri-information-line me-1 text-primary"></i> معلومات النسخة</h6>
                    </div>
                    <div class="card-body">
                ...
                <div id="actions-section" style="display: {{ $backup->status === 'completed' ? 'block' : 'none' }};">
                    <div class="card custom-card form-card mb-4">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h6 class="mb-0 fw-semibold fs-15"><i class="ri-tools-line me-1 text-primary"></i> الإجراءات</h6>
                        </div>
                        <div class="card-body">
                            <div class="d-flex gap-2">
                                ...
                                @can('backup-restore')
                                <form id="restore-form" action="{{ route('admin.backups.restore', $backup->id) }}" method="POST" class="d-inline-flex flex-wrap gap-2 align-items-center">
                                    @csrf
                                    <select name="restore_mode" id="restore_mode" class="form-select form-select-sm" style="width:auto;">
                                    ...
                                    <button type="submit" class="btn btn-warning btn-sm" id="restore-btn">
                ...
                <div class="card custom-card data-table-card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h6 class="mb-0 fw-semibold fs-15">
                            <i class="ri-list-check-2 me-1 text-primary"></i> سجل العمليات
                ...
                <div class="card custom-card data-table-card mb-4 border-warning">
                    <div class="card-header d-flex justify-content-between align-items-center bg-warning bg-opacity-10">
                        <h6 class="mb-0 fw-semibold fs-15">
                            <i class="ri-history-line me-1 text-warning"></i> تقرير الاستعادة
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (restoreForm) {
        restoreForm.addEventListener('submit', function(e) {
            ...
            const backupName = @json($backup->name);
            const typed = prompt('للتأكيد، اكتب اسم النسخة بالضبط:\n' + backupName);
            if (typed !== backupName) {
                alert('لم يتم التأكيد. ألغيت العملية.');
                return;
            }
            ...
            fetch(restoreForm.action, {
                ...
                body: JSON.stringify({
                    confirm: true,
                    restore_mode: document.getElementById('restore_mode')?.value || 'full',
                    restore_confirm_name: backupName
                })
            })
            .then(response => {
    ...
    function startRestorePolling(backupId) {
        ...
        restorePollingInterval = setInterval(() => {
            fetch(`{{ route('admin.backups.restore-status', $backup->id) }}`, {
                method: 'GET',
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest',
            ...
            .then(data => {
                const restoreStatus = data.restore_status || 'running';
                updateRestoreProgress(data.logs || [], restoreStatus);
                ...
                if (restoreStatus === 'completed' || restoreStatus === 'failed') {
                    ...
                    if (restoreStatus === 'completed') {
                        ...
                        if (statusDiv && data.restore_error_message) {
                            statusDiv.textContent = data.restore_error_message;
                        }
                        if (progressBar) {
                            progressBar.classList.remove('bg-warning');
