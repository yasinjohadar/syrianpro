...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('app_storage_configs', function (Blueprint $table) {
            ...
            $table->enum('driver', ['local', 's3', 'google_drive', 'dropbox', 'azure', 'ftp', 'sftp', 'digitalocean', 'wasabi', 'backblaze', 'cloudflare_r2', 'bunny'])->comment('نوع التخزين');
            $table->text('config')->comment('الإعدادات المشفرة');
            $table->boolean('is_active')->default(true);
            $table->integer('priority')->default(0)->comment('الأولوية');
            $table->integer('max_backups')->nullable()->comment('الحد الأقصى للنسخ الاحتياطية');
